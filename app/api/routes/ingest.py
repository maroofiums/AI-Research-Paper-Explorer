...
from app.models.schemas import IngestPaper, IngestRequest, IngestResponse, Paper
...
async def _ingest_one(paper: Paper) -> IngestPaper:
...
async def ingest_arxiv(request: IngestRequest) -> IngestResponse:
    ...
    if not request.arxiv_id and not request.query:
        raise HTTPException(
            status_code=400,
            detail="Provide either 'arxiv_id' or 'query'",
        )
